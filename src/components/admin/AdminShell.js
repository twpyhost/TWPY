...
export default function AdminShell({
  ...
  // item del nav del panel, debajo de Sistema. La navbar publica no lo tiene.
  const logout = async () => {
    try {
  ...
  return (
    <div className="flex min-h-screen bg-black text-white">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-[60] flex w-[250px] flex-none flex-col overflow-y-auto border-r border-white/[.08] bg-black transition-transform duration-300 max-[899px]:z-[60] min-[900px]:static min-[900px]:z-auto min-[900px]:h-auto min-[900px]:translate-x-0 ${
          drawerOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between gap-2.5 border-b border-white/[.08] px-[18px] py-[18px] pt-[22px]">
          <div className="flex flex-col gap-0.5">
            <span className="font-display text-[25px] italic leading-none tracking-[0.02em]">
              TEKKEN PY
            </span>
            <span className="font-body text-[10px] font-bold tracking-[0.22em] text-primary-500">
              PANEL ADMIN
            </span>
          </div>
          <button
        ...
        <nav className="flex flex-col gap-0.5 py-3">
          ...
          <button
            type="button"
            onClick={logout}
            className="flex w-full items-center gap-[11px] border-l-[3px] border-l-transparent bg-transparent px-4 py-3 text-left font-display text-lg uppercase tracking-[0.05em] text-white/50 transition-colors duration-200 hover:border-l-error hover:bg-error/[.12] hover:text-error"
          >
            <svg
              viewBox="0 0 24 24"
              width="18"
            ...
            <span className="flex-1">Logoff</span>
          </button>
        </nav>
